...
if(strlen($_SESSION['alogin'])==0) {   
    ...
    if(isset($_POST['update'])) {
        ...
        // Validate inputs
        if (!isValidInput($bookname) || !isValidInput($isbn) || !isValidInput($price)) {
            echo "<script>alert('Invalid input. HTML tags are not allowed.');</script>";
            echo "<script>window.location.href='manage-books.php'</script>";
            exit();
        }

        // Sanitize inputs to prevent XSS
        $bookname = htmlspecialchars($bookname, ENT_QUOTES, 'UTF-8');
            ...
            <div class="row">
                <div class="col-md-12 col-sm-12 col-xs-12">
                    <div class="panel panel-primary" style="margin-top: 50px; border-radius: 10px; border-color: #9B00EA;">
                        ...
                        <i class="fa-solid fa-book"></i> BOOK INFO
                        </div>
                        <div class="panel-body">
                            <form role="form" method="post">
                                <?php 
                                $bookid = intval($_GET['bookid']);
                                $sql = "SELECT books.BookName, category.CategoryName, category.id as cid, authors.AuthorName, authors.id as athrid, books.ISBNNumber, books.BookPrice, books.id as bookid, books.bookImage 
                                        FROM books 
                                        JOIN category ON category.id = books.CatId 
                                ...
                                <div class="col-md-6">
                                    <div class="form-group">
                                        ...
                                        <input class="form-control" type="text" name="price" value="<?php echo htmlentities($result->BookPrice); ?>" required="required" oninput="this.value = this.value.replace(/[^0-9.]/g, '')"/>
                                    </div>
                                </div>
